...
class Scaler():
    def __init__(self):
    	self.dmin = pd.Series(dtype=float)
    	self.dmax = pd.Series(dtype=float)
...
def read_data(scaler):
	'''
	Read the train, dev, and test datasets
	'''

	df = pd.read_csv(
		'dataset/train.csv', skipinitialspace=True)
	train_input = scaler(df.iloc[:,1:], True).to_numpy().astype(float)

	train_target = df.iloc[:,0].to_numpy().astype(int)
	...
	df = pd.read_csv(
		'dataset/dev.csv', skipinitialspace=True)
	dev_input = scaler(df.iloc[:,1:], False).to_numpy().astype(float)

	dev_target = df.iloc[:,0].to_numpy().astype(int)
	...
	df = pd.read_csv(
		'dataset/test.csv', skipinitialspace=True)
	test_input = scaler(df, False).to_numpy().astype(float)
...
def main():
	...
	scaler = Scaler()
	train_input, train_target, dev_input, dev_target, test_input = read_data(scaler)
